...
export default function Slider() {
  ...
  const [sliderRef, instanceRef] = useKeenSlider({
    ...
    slideChanged(slider) {
      ...
      var counting = slider.track.details.rel+1;
      counting == 7 ? counting = 0 : counting = counting;
      console.log("slideChanged rel",counting);
      setCurrentSlide(slider.track.details.rel);
      setActiveVideo(slider.track.details.rel);
      setloopCount(counting)
      // instanceRef.current?.moveToIdx(counting);
    },
    created() {
      setLoaded(true);
    },
  ...
  const handleVideo = (id) => {
    console.log(" handleVideo id", id);
    setActiveVideo(id);
    id === 6 ? setloopCount(0) : setloopCount(id + 1);
    // console.log(id);
    instanceRef.current?.moveToIdx(id);
  };

  useEffect(() => {
    console.log("useEffect loop count",loopCount);
